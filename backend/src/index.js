...
// y activa RLS en planillas/planilla_detalle — ver backend/src/db/rlsSetup.js
// para el detalle de ambos pasos y por qué corren secuenciales (no en
// paralelo: causaron un deadlock real, ver ERRORES-CONOCIDOS.md 2026-08-01).
// No bloqueante: rlsSetup ya atrapa el error de cada paso por separado y
// solo relanza un resumen si algo falló — acá simplemente lo logueamos sin
// tumbar el arranque del servidor (mismo patrón que el resto de este
// archivo). backend/src/test-setup/globalSetup.js llama a esta misma
// función en CI, pero ahí SÍ deja que el error se propague.
migrarRolTenantScopedYRlsPlanillas().catch(err => {
  console.warn('   Esquema:     (Aviso) No se pudo verificar rol app_tenant_scoped / RLS de planillas:', err.message)
})
