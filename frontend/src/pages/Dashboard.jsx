...
import { LayoutDashboard, ChefHat, Package, AlertTriangle, TrendingUp, DollarSign, ShoppingBag } from 'lucide-react'
...
export default function Dashboard() {
  ...
  const recetasConDatos = Object.values(recetas).map(r => {
    const ct = r.ingredientes?.reduce((s, i) => {
      return s + convertir(i.cantidad, i.unidad, i.unidad_inventario) * (i.precio || 0)
    }, 0) || 0
  ...
  return (
    <div className="space-y-4 max-w-4xl">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        <div className="kpi-card"><div className="text-xs text-gray-400 mb-1 flex items-center gap-1"><ShoppingBag size={11} /> Ventas hoy</div><div className="text-2xl font-semibold">{ventasHoy}</div><div className="text-xs text-gray-400 mt-1">transacciones</div></div>
        <div className="kpi-card"><div className="text-xs text-gray-400 mb-1 flex items-center gap-1"><DollarSign size={11} /> Ingresos hoy</div><div className="text-2xl font-semibold text-green-700">{fmt(ingresosHoy)}</div><div className="text-xs text-gray-400 mt-1">ventas del día</div></div>
        <div className="kpi-card"><div className="text-xs text-gray-400 mb-1 flex items-center gap-1"><Package size={11} /> Stock crítico</div><div className="text-2xl font-semibold text-red-600">{stockBajo.length}</div><div className="text-xs text-red-500 mt-1">{stockBajo.length > 0 ? 'reabastecer' : 'stock OK'}</div></div>
      </div>
      ...
      {topRentables.length > 0 && (
        <div className="card">
          <h3 className="text-sm font-medium text-gray-600 mb-4 flex items-center gap-2"><TrendingUp size={14} /> Rentabilidad por producto</h3>
                ...
                <div key={r.producto}>
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ color, background: bg }}>{r.margen.toFixed(1)}%</span>
                    </div>
                  </div>
      ...
      {stockBajo.length > 0 && (
        <div className="card" style={{ borderColor: '#fca5a5' }}>
          <h3 className="text-sm font-medium text-red-600 mb-3 flex items-center gap-2"><Package size={14} /> Insumos con stock crítico</h3>
          <div className="space-y-1.5">
            {stockBajo.map(i => (
