  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold text-text-default">Caja de Producción</h1>
          <p className="text-xs text-text-muted">Registro de hornadas y ventas del día</p>
        </div>
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <div className="flex gap-4 text-sm">
          <span className="text-text-muted">Vendido: <strong className="text-status-success">C$ {totalDia.toFixed(2)}</strong></span>
          <span className="text-text-muted">Merma total: <strong className="text-status-danger">{totalMerma} uds</strong></span>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-semibold text-text-default">Nueva hornada</h2>
              <button onClick={() => setModal(false)}><X size={18} className="text-text-muted" /></button>
            </div>
...
function LoteCard({ lote, editando, onEditar, onGuardar, onCancelar, onEliminar }) {
  ...
  return (
    <div className={`bg-white rounded-xl border p-4 ${lote.cerrado ? 'border-status-success/40 bg-success-light/30' : 'border-border-default'}`}>
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2">
            <Package size={15} className="text-text-muted" />
            <span className="font-medium text-text-default">{lote.producto}</span>
            {lote.cerrado && <span className="text-xs px-2 py-0.5 rounded-full bg-success-light text-status-success-fg">Cerrado</span>}
          </div>
        ...
        <div className="flex gap-1">
          {!lote.cerrado && !editando && (
            <button onClick={onEditar} className="text-xs px-2 py-1 rounded-lg bg-border-default hover:bg-text-muted/20 text-text-subtle">Actualizar</button>
          )}
          <button onClick={onEliminar} className="p-1 text-text-muted hover:text-status-danger"><X size={14} /></button>
        </div>
      </div>
      ...
      {editando && (
        <div className="mt-3 pt-3 border-t border-border-default grid grid-cols-3 gap-2">
          <div>
            <label className="text-xs text-text-muted">Vendidas</label>
            <input type="number" value={vendida} onChange={e => setVendida(e.target.value)} className="w-full text-sm border border-border-default rounded-lg px-2 py-1 mt-0.5" />
          </div>
          <div>
            <label className="text-xs text-text-muted">Merma</label>
            <input type="number" value={merma} onChange={e => setMerma(e.target.value)} className="w-full text-sm border border-border-default rounded-lg px-2 py-1 mt-0.5" />
          </div>
          <div>
            <label className="text-xs text-text-muted">Precio C$</label>
            <input type="number" step="0.01" value={precio} onChange={e => setPrecio(e.target.value)} className="w-full text-sm border border-border-default rounded-lg px-2 py-1 mt-0.5" />
          </div>
          <div className="col-span-3 flex gap-2 mt-1">
            <button onClick={() => onGuardar({ cantidad_vendida: Number(vendida), cantidad_merma: Number(merma), precio_unitario: Number(precio) })}
              className="flex-1 flex items-center justify-center gap-1 text-xs py-1.5 rounded-lg bg-status-success text-white hover:opacity-90">
            ...
            <button onClick={() => onGuardar({ cantidad_vendida: Number(vendida), cantidad_merma: Number(merma), precio_unitario: Number(precio), cerrado: true })}
              className="flex-1 text-xs py-1.5 rounded-lg bg-brand-ink text-white hover:opacity-90">
              Cerrar caja
            </button>
            <button onClick={onCancelar} className="px-3 text-xs py-1.5 rounded-lg bg-border-default text-text-subtle">Cancelar</button>
          </div>
        </div>
      )}

      {!editando && (
        <p className="text-xs font-medium text-status-success-fg mt-2">
